...
const query = gql`
  query($start: DateTime!, $end: DateTime!, $after: String, $first: Int!, $appid: ID!) {
    app(id: $appid) {
      id
      name
      events(
...
const eventMessageMappings = {
  ...
  'SUBSCRIPTION_CHARGE_CANCELED':
    'has cancelled their subscription :tired_face:',
...
const processEvents = async (slackClient, events) => {
  for (let event of events) {
    ...
    await slackClient.messages.create({
      ...
      text: `${event.node.shop.myshopifyDomain} ${eventMessageMappings[event.node.type]}`,
    });
  }
}
...
module.exports.main = async (graphqlClient, slackClient, kv) => {
  ...
  try {
    ...
    if (data.app.events.pageInfo.hasNextPage) {
      console.warn(`Too many event to process, consider running more frequenty`); // could paginate, but firing >100 events off at once is probably too many already
    }
